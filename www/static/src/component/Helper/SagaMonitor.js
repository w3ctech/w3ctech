/* eslint-disable no-console, no-use-before-define, no-nested-ternary */
...
function logFormatter() {
  ...
  function add(msg, ...args) {
    ...
    if (!IS_BROWSER && typeof msg === 'string') {
      const prevMsg = msg;
      msg = msg.replace(/^%c\s*/, ''); // eslint-disable-line no-param-reassign
...
const logSaga = (...topEffects) => {
  if (!topEffects.length) {
    topEffects = rootEffects; // eslint-disable-line no-param-reassign
  }
